...
public class JsDocParser {
	...
	private static final Pattern PATTERN_PARAM = Pattern
			.compile("@param\\s+(.*)");
	private static final Pattern PATTERN_RETURN = Pattern
			.compile("@return\\s+(.*)");
	private static final Pattern PATTERN_RETURNS = Pattern
			.compile("@returns\\s+(.*)");
	private static final Pattern PATTERN_TYPE = Pattern
			.compile("@type\\s+(.*)");
...

			{
				...
				if (matcher.matches()) {
					...
					for (int i = 0; i < dim.length; i++) {
						if (param.typeList.isEmpty() && dim[i].startsWith("{")
								&& dim[i].endsWith("}")) {
							String typeStr = dim[i].substring(1,
									dim[i].length() - 1);
				...
				if (matcher.matches()) {
					...
					for (int i = 0; i < dim.length; i++) {
						if (jsDoc.returnTypeList.isEmpty()
								&& dim[i].startsWith("{")
								&& dim[i].endsWith("}")) {
							updateReturnType(jsDoc,
									dim[i].substring(1, dim[i].length() - 1));
						} else {
							desc.append(dim[i]).append(" ");
						}
					}

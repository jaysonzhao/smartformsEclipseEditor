...
/**
 * This is a function element for JavaScript.
 * 
 * @author shinsuke
 * 
 */
public class JavaScriptFunction extends JavaScriptElement {
	...
	public JavaScriptArgument[] getArguments() {
		return argumentList
				.toArray(new JavaScriptArgument[argumentList.size()]);
	}
	...
	public boolean hasStaticProperties() {
		...
		for (JavaScriptElement jsElement : context.elementMap.values()) {
			if (!"prototype".equals(jsElement.getName())
					&& !"arguments".equals(jsElement.getName())) {
				return true;
			}
		}
		return false;

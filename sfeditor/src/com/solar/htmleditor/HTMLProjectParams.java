...
public class HTMLProjectParams {
	...
	public void save(IProject project) throws Exception {
		IFile configFile = project.getFile(".solar");
		...
		props.put(P_JAVA_SCRIPT_V_FILE_OVERVIEW_TAGS,
				javaScriptFileoverviewTags);
		...
		props.put(P_JAVA_SCRIPT_V_NON_STANDARD_JSDOC,
				javaScriptNonStandardJsDocs);
		props.put(P_JAVA_SCRIPT_V_STRICT_MODULE_DEP_CHECK,
				javaScriptStrictModuleDepCheck);
		...
		props.put(P_JAVA_SCRIPT_F_INDENT_SIZE,
				Integer.toString(javaScriptIndentSize));
		props.put(P_JAVA_SCRIPT_F_INDENT_CHAR,
				Character.toString(javaScriptIndentChar));
		props.put(P_JAVA_SCRIPT_F_PRESERVE_NEWLINES,
				Boolean.toString(javaScriptPreserveNewlines));
		props.put(P_JAVA_SCRIPT_F_INIT_INDENT_LEVEL,
				Integer.toString(javaScriptInitIndentLevel));
		props.put(P_JAVA_SCRIPT_F_SPACE_AFTER_ANON_FUNC,
				Boolean.toString(javaScriptSpaceAfterAnonFunc));
		props.put(P_JAVA_SCRIPT_F_BRACES_ON_OWN_LINES,
				Boolean.toString(javaScriptBracesOnOwnLine));
	...
	public void load(IProject project) throws Exception {
		...
		if (configFile.exists()) {
			...
			javaScriptAccessControls = props
					.getProperty(P_JAVA_SCRIPT_V_ACCESS_CONTROLL);
			if (javaScriptAccessControls == null) {
				javaScriptAccessControls = "Off";
			}
			javaScriptCheckRegExp = props
					.getProperty(P_JAVA_SCRIPT_V_CHECK_REG_EXP);
			if (javaScriptCheckRegExp == null) {
				javaScriptCheckRegExp = "Off";
			}
			javaScriptCheckTypes = props
					.getProperty(P_JAVA_SCRIPT_V_CHECK_TYPES);
			...
			javaScriptDeprecated = props
					.getProperty(P_JAVA_SCRIPT_V_DEPRECATED);
			if (javaScriptDeprecated == null) {
				javaScriptDeprecated = "Off";
			}
			javaScriptFileoverviewTags = props
					.getProperty(P_JAVA_SCRIPT_V_FILE_OVERVIEW_TAGS);
			if (javaScriptFileoverviewTags == null) {
				javaScriptFileoverviewTags = "Off";
			}
			javaScriptInvalidCasts = props
					.getProperty(P_JAVA_SCRIPT_V_INVALID_CASTS);
			if (javaScriptInvalidCasts == null) {
				javaScriptInvalidCasts = "Off";
			}
			javaScriptMissingProperties = props
					.getProperty(P_JAVA_SCRIPT_V_MISSING_PROP);
			if (javaScriptMissingProperties == null) {
				javaScriptMissingProperties = "Off";
			}
			javaScriptNonStandardJsDocs = props
					.getProperty(P_JAVA_SCRIPT_V_NON_STANDARD_JSDOC);
			if (javaScriptNonStandardJsDocs == null) {
				javaScriptNonStandardJsDocs = "Off";
			}
			javaScriptStrictModuleDepCheck = props
					.getProperty(P_JAVA_SCRIPT_V_STRICT_MODULE_DEP_CHECK);
			if (javaScriptStrictModuleDepCheck == null) {
				javaScriptStrictModuleDepCheck = "Off";
			}
			javaScriptUndefinedVars = props
					.getProperty(P_JAVA_SCRIPT_V_UNDEFINED_VARS);
			if (javaScriptUndefinedVars == null) {
				javaScriptUndefinedVars = "Off";
			}
			javaScriptUnknownDefines = props
					.getProperty(P_JAVA_SCRIPT_V_UNKNOWN_DEFINES);
			if (javaScriptUnknownDefines == null) {
				javaScriptUnknownDefines = "Off";
			}
			javaScriptVisibility = props
					.getProperty(P_JAVA_SCRIPT_V_VISIBILITY);
			if (javaScriptVisibility == null) {
				javaScriptVisibility = "Off";
			}
			javaScriptUseUndefined = props
					.getProperty(P_JAVA_SCRIPT_V_USE_UNDEFINED);
			...
			preserveNewlines = props
					.getProperty(P_JAVA_SCRIPT_F_PRESERVE_NEWLINES);
			if (preserveNewlines == null) {
				preserveNewlines = "true";
			}
			initIndentLevel = props
					.getProperty(P_JAVA_SCRIPT_F_INIT_INDENT_LEVEL);
			if (initIndentLevel == null) {
				initIndentLevel = "0";
			}
			spaceAfterAnonFunc = props
					.getProperty(P_JAVA_SCRIPT_F_SPACE_AFTER_ANON_FUNC);
			if (spaceAfterAnonFunc == null) {
				spaceAfterAnonFunc = "false";
			}
			bracesOnOwnLine = props
					.getProperty(P_JAVA_SCRIPT_F_BRACES_ON_OWN_LINES);
			if (bracesOnOwnLine == null) {
				bracesOnOwnLine = "true";
			}
		} else {
			// for old versions
			this.root = project.getPersistentProperty(new QualifiedName(
					HTMLPlugin.getDefault().getPluginId(), P_ROOT));
			useDTD = project.getPersistentProperty(new QualifiedName(HTMLPlugin
					.getDefault().getPluginId(), P_USE_DTD));
		...
		this.detectTaskTag = project
				.hasNature(HTMLProjectNature.HTML_NATURE_ID);
		...
		this.javaScriptIndentChar = indentChar.length() > 0 ? indentChar
				.charAt(0) : ' ';
		try {
			this.javaScriptPreserveNewlines = Boolean
					.parseBoolean(preserveNewlines);
		} catch (Exception e) {
		}
		try {
		...
		try {
			this.javaScriptSpaceAfterAnonFunc = Boolean
					.parseBoolean(spaceAfterAnonFunc);
		} catch (Exception e) {
		}
		try {
			this.javaScriptBracesOnOwnLine = Boolean
					.parseBoolean(bracesOnOwnLine);
		} catch (Exception e) {
		}
	}
	...
	public void setJavaScriptSpaceAfterAnonFunc(
			boolean javaScriptSpaceAfterAnonFunc) {
	...
	public void setJavaScriptMissingProperties(
			String javaScriptMissingProperties) {
		this.javaScriptMissingProperties = javaScriptMissingProperties;
	}
	...
	public void setJavaScriptNonStandardJsDocs(
			String javaScriptNonStandardJsDocs) {
		this.javaScriptNonStandardJsDocs = javaScriptNonStandardJsDocs;
	}
	...
	public void setJavaScriptStrictModuleDepCheck(
			String javaScriptStrictModuleDepCheck) {
		this.javaScriptStrictModuleDepCheck = javaScriptStrictModuleDepCheck;
	}
